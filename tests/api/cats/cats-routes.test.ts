...
describe('cats API routes', () => {
  ...
  it('scopes cat deletion by authenticated user id and cat id', async () => {
    ...
    const response = await deleteCatRoute(new Request('http://test.local') as never, params('cat-1'))
  ...
  it('returns 404 when scoped cat deletion finds no row', async () => {
    ...
    const response = await deleteCatRoute(new Request('http://test.local') as never, params('missing-cat'))
