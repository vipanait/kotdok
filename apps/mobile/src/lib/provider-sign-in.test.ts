...
describe('signing in with a provider', () => {
  it('exchanges the returned code for a session', async () => {
    ...
    await expect(createProviderSignIn(d)('custom:yandex', messages)).resolves.toEqual({ kind: 'session' })
  ...
  it('treats a closed browser as a cancellation, not a failure', async () => {
    ...
    await expect(createProviderSignIn(d)('google', messages)).resolves.toEqual({ kind: 'cancelled' })
  ...
  it('fails when the provider refuses, and hands the reason to the log', async () => {
    ...
    expect((await createProviderSignIn(d)('google', messages)).kind).toBe('failed')
    expect(d.exchangeCode).not.toHaveBeenCalled()
  ...
  it('fails when the return carries no code at all', async () => {
    ...
    expect((await createProviderSignIn(d)('custom:yandex', messages)).kind).toBe('failed')
    expect(d.exchangeCode).not.toHaveBeenCalled()
  })
  ...
  it('tells the user something readable rather than the provider’s wording', async () => {
    ...
    const outcome = await createProviderSignIn(d)('google', messages)
