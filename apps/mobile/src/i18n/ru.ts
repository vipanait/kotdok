...
export const ru = {
  ...
  medicalRecord: {
    ...
    visits: {
      ...
      heldReadOnly: 'Визит состоялся. Запись хранится в истории, изменить её нельзя — только удалить, если она ошибочная. Назначение можно добавить в лекарства.',
    ...
    earlierDone: (day: string) =>
      `Эта позиция уже была отмечена сделанной раньше — ${day}. Новые данные не сохранены: выполненную запись изменить нельзя. Откройте её в медкарте и проверьте.`,
    earlierNext: (day: string) =>
      `Эта позиция уже была отмечена сделанной раньше (${day}) с другой следующей датой. Новые данные не сохранены: выполненную запись изменить нельзя. Откройте её в медкарте и проверьте.`,
  ...
  vetSummary: {
    htmlLang: 'ru',
    checkColumns: ['Дата', 'Срочность', 'Что было'] as [string, string, string],
    weightColumns: ['Дата', 'Вес'] as [string, string],
    title: 'Для врача',
    entry: 'Для врача',
    pdfTitle: (name: string) => `Медкарта: ${name}`,
    pet: 'Питомец',
    important: 'Важно знать',
    notStated: 'Не указано владельцем',
    weightFromForm: (kg: string) => `${kg} — из анкеты, без даты`,
    weightOn: (kg: string, day: string) => `${kg} · ${day}`,
    vaccinations: 'Прививки',
    vaccinationColumns: ['Болезнь', 'Последняя', 'Препарат', 'Следующая'] as [string, string, string, string],
    parasites: 'Паразиты',
    parasiteColumns: ['От чего', 'Последняя', 'Средство', 'Следующая'] as [string, string, string, string],
    parasiteGroups: { fleas_ticks: 'Блохи и клещи', worms: 'Глисты' },
    noRecord: 'Нет записей',
    overdueSince: (day: string) => `Просрочено с ${day}`,
    visits: 'Визиты за последний год',
    visitColumns: ['Дата', 'Вид', 'Диагноз', 'Назначения'] as [string, string, string, string],
    noVisits: 'Визитов за год не записано',
    weight: 'Вес',
    noWeights: 'Измерений нет',
    checks: 'Последние проверки в «Лапке»',
    noChecks: 'Проверок не было',
    footer: (day: string) => `Составлено владельцем в приложении «Лапка» ${day}. Не является ветеринарным документом.`,
    page: 'Стр.',
    send: 'Отправить PDF',
    preparing: 'Готовим PDF…',
    failed: 'Не удалось подготовить PDF',
    retry: 'Повторить',
  ...
  reminders: {
    ...
    askBody: (days: number) =>
      `Пришлём уведомление за ${days === 7 ? 'неделю' : `${days} ${plural(days, 'день', 'дня', 'дней')}`} и в день срока. Выключить можно в профиле.`,
    remind: 'Напоминать',
    notNow: 'Не сейчас',
