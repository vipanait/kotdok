...
describe('recording vaccinations', () => {
  ...
  it('returns the same record for a repeated request instead of a second one', async () => {
    ...
    const first = HealthEventSchema.parse(await (await create(body, 'key-1')).json())
    const second = HealthEventSchema.parse(await (await create(body, 'key-1')).json())
...
describe('marking a plan done', () => {
  ...
  it('moves only the item marked done and keeps the other planned (MR-03.2)', async () => {
    ...
    const response = await completeItem(
      request(tokenA, 'POST', { done_on: day(0), next_on: day(365) }, 'done-1'),
  ...
  it('does not add anything when the same "done" is sent again (MR-03.2)', async () => {
    ...
    await completeItem(request(tokenA, 'POST', body, 'done-2'), itemParams(pet, plan.items[0].id))
    const before = await liveEvents()

    await completeItem(request(tokenA, 'POST', body, 'done-2'), itemParams(pet, plan.items[0].id))
    await completeItem(request(tokenA, 'POST', body, 'done-3'), itemParams(pet, plan.items[0].id))
...
describe('correcting, cancelling and listing', () => {
  ...
  it('does not touch the form’s vaccination answer', async () => {
    await db.query(`update public.pets set vaccinated = false where id = $1`, [pet])
    await create({ kind: 'vaccination', status: 'done', date: day(-1), items: [rabies] })
    const { rows } = await db.query(`select vaccinated from public.pets where id = $1`, [pet])
    expect(rows[0].vaccinated).toBe(false)
  })
