...
function AppleButton({ onClick, loading, label }: { onClick: () => void; loading: boolean; label: string }) {
  return <OAuthButton onClick={onClick} loading={loading} label={label} icon={<AppleIcon />} />
}
...
/**
 * Apple's mark, sized like the Google and Yandex icons beside it.
 *
 * The shape is Apple's own (Apple Design Resources,
 * `docs/design/mobile-concept-v1/assets/apple-logo-black.svg`), with the
 * viewBox tightened to it, which crops the padding Apple builds into the file.
 * Apple's rules forbid the crop and ask for a larger title; the owner chose on
 * 17 September 2026 to match the other sign-in buttons, as in the app.
 */
function AppleIcon() {
  return (
    <svg width="15" height="18" viewBox="7.75 10.5 15.4609 19" aria-hidden="true" className="shrink-0">
