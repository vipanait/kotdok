...
| Чем запускать worker чаще раза в сутки | Фоновая обработка анализа | **Этап 6**. Времени выполнения хватает — 300 с; не хватает планировщика. Варианты: Pro (cron раз в минуту), внешний планировщик, или `pg_cron` в самой Supabase. Выбор за владельцем |
...
Пробная задача worker не запускалась. Время выполнения не является препятствием: 300 секунд достаточно для анализа. Препятствие — планировщик: на Hobby cron допускает лишь один запуск в сутки с точностью ±59 минут, чего для очереди задач недостаточно.
