...
`update:production` — то же для App Store. Скрипт [`publish-update.mjs`](../../apps/mobile/scripts/publish-update.mjs) отказывается работать с незакоммиченными изменениями, собирает бандл с переменными production и очищенным кэшем Metro, проверяет его `check-bundle.mjs --target store` (есть production-проект и `https://lapka.my`, нет staging и адресов `192.168.`) и отправляет ровно проверенный экспорт. Установленное приложение ищет обновление при запуске и при возврате из фона (не чаще раза в минуту), скачивает его молча и спрашивает «Приложение обновилось — перезапустить?» ([`UpdatePrompt.tsx`](../../apps/mobile/src/features/updates/UpdatePrompt.tsx)). Пока на экране несохранённая форма, вопрос ждёт. «Позже» оставляет обновление до следующего запуска и больше не спрашивает про это же обновление. В профиле под номером версии строка «Обновление от … · первые 8 символов id» — по ней видно, что на телефоне именно отправленное обновление; для кода, вшитого в сборку, строки нет.

`expo-updates` — нативный модуль: dev-сборку на телефоне после этого изменения нужно пересобрать, старая упадёт при загрузке JS из Metro.
